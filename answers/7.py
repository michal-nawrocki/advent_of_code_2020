# Input with removed "." from end of line
# helps with parsing
...
def _get_dict_of_bags():
    """
        Return a dict of bag association, where for each key there is a list of
        (bag_color, how_many_can_fit) tuples
    """
    dict_of_bags = dict()

    for line in small:
        contain_index = line.find(" contain")
        main_bag = line[0:contain_index-5]

        if line[contain_index:] == " contain no other bags":
            dict_of_bags[main_bag] = []
        else:
            sub_bags = line[contain_index+9:].split(", ")
            value_for_dict = []
            for bag_str in sub_bags:
                number_split = bag_str.find(" ")
                count_of_bags = int(bag_str[:number_split])
                value_for_dict.append((' '.join(i for i in bag_str.split()[1:-1]), count_of_bags))

            dict_of_bags[main_bag] = value_for_dict

    return dict_of_bags
    

def _can_bag_contain_gold_bag(bag_color, dict_of_bags):
    look_ups = dict_of_bags.get(bag_color, [])
    all_returned = []
    
    for look_up in look_ups:
        if look_up[0] == "shiny gold":
            return True
        else:
            all_returned.append(_can_bag_contain_gold_bag(look_up[0], dict_of_bags))
    
    if True in all_returned:
        return True
    else:
        return False

def _get_all_bags_inside_shiny_gold(bag_color, dict_of_bags):
    look_ups = dict_of_bags.get(bag_color, [])
    count_of_bags = 0
    ...
    for bag in look_ups:
        ...
        count_of_bags += bag[1] * _get_all_bags_inside_shiny_gold(bag[0], dict_of_bags) + bag[1]

    return count_of_bags

def part_one():
    shiny_gold_bag_count = 0
    dict_of_bags = _get_dict_of_bags()

    for bag_color in dict_of_bags.keys():
        if _can_bag_contain_gold_bag(bag_color, dict_of_bags):
            shiny_gold_bag_count += 1

    print(shiny_gold_bag_count)

def part_two():
    fit_inside_shiny_gold = 0
    dict_of_bags = _get_dict_of_bags()

    print(_get_all_bags_inside_shiny_gold("shiny gold", dict_of_bags))
